...
class ConsolePresenter:
    ...
    def show_session_list(self, sessions: list[SessionRecord]) -> None:
        if not sessions:
            self._print("[SaiWorks] no saved sessions")
            return

        self._print("[SaiWorks] saved sessions:")
    ...
    def show_help(self, registry=None) -> None:
        ...
        self._print(f"{BOLD}SaiWorks CLI Workbench Shortcuts & Commands:{RESET}")
    ...
    def show_interrupted(self) -> None:
        RED = "\033[1;31m"
        RESET = "\033[0m"
        self._print(f"\n {RED}⎿  Interrupted · What should SaiWorks CLI do instead?{RESET}\n")
