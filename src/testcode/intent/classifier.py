...
ENGLISH_PROJECT_ACTION_TERMS = {
    "add", "build", "change", "create", "delete", "edit", "fix", "generate",
    "implement", "modify", "move", "patch", "remove", "rename", "replace",
    "refactor", "scaffold", "update", "upgrade", "write",
    "debug",
    "review",
    "test",
}
...
class RequestIntentClassifier:
    """Classify context-loading needs; write evidence is an explicit contract."""
    ...
    def classify(self, prompt: str, metadata: dict | None = None) -> RequestIntent:
        ...
        # Natural-language intent is decided by the model through its tool use
        # and terminal response.  Requiring a workspace change is reserved for
        # callers with a real execution contract (for example, a delegated
        # implementation task), rather than inferred from words in a prompt,
        # URL, log, or stack trace.
        file_changes = change_override if isinstance(change_override, bool) else False
