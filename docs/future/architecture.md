...
- 复用 `SaiWorks` 已有能力仓库、工具、安全、日志和会话基础设施。
...
### Agent Plane：`SaiWorks`

当前 `SaiWorks` 是 LLM 驱动的 CLI workbench，已有工具执行、能力仓库、Skill、MCP、安全审批、
...
Device Plane 负责设备注册、能力、租约、调度、执行、状态和数据协调。第一版可以是 `SaiWorks`
...
当前保持 Python `SaiWorks` 与 Node/Electron API Control Board 两个仓库和独立进程。未来组件先
