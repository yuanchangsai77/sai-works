...
本文档描述当前交互式 TTY 的实际行为。`--once`、重定向、CI 和非 TTY 环境继续使用 `ConsolePresenter`
的逐行输出；启用模型流式传输时，它会直接写出安全自然语言增量，并在重试前明确标记已显示但未采纳的
部分响应。交互式会话使用 `TUIConsolePresenter` 的原生终端 backend。
...
模型自然语言在生成期间显示为可清除的动态预览，预览高度根据当前终端可用空间计算，而不是固定截取
最后几行；随着内容增加，旧行自然向上移动。完整响应通过校验后，工具轮次的自然语言也会清除预览，
并通过与最终回答相同的 Markdown renderer 写入原生 scrollback；最终回答则在工具 transcript 提交后
使用同一路径稳定呈现。
这样同时保留生成进度与稳定排版，且不会把工具记录放到最终回答之后。工具名称、参数和动作仍等待完整
响应校验，不进入自然语言预览路径。

每个稳定的模型自然语言区块与前一项可见内容之间恰好保留一行空白。TUI 会复用提示框或前一模型区块
已有的尾部分隔；当前一项是没有尾部空行的工具记录时，Markdown renderer 才补充顶部留白。因此首轮
说明、中间轮次说明和最终回答具有一致间距，也不会叠加成两行。
...
显示顺序遵循真实的模型轮次与执行因果：一轮完整且有效的自然语言先稳定渲染，随后按运行时实际执行
顺序提交该轮工具，再进入下一模型轮。单个模型回复可以包含多个工具动作；这些动作必须整体解析并校验
后才能执行，因此不会根据旁白内容猜测映射关系或人为拆散成额外模型轮次。
...
- `ConsolePresenter`：非 TTY 和兼容路径，负责自然语言增量、重试丢弃提示和最终 Markdown；已完整流式
  显示的最终正文不会重复打印。
