...
工作区授权与任务 checkpoint 分离。每个持久化会话保存初始工作区、当前活动工作区和已批准的
外部路径；初始工作区始终是该 session 的有效访问根，即使活动工作区后来切换。恢复同一 session
时会恢复这些边界。`/new` 与 `/compact` 创建新 session 时从原始工作区开始，不继承旧 session 的
外部授权或活动工作区。

访问工作区外的路径时，运行时先请求一次针对该真实路径的授权。批准后，该路径加入本会话的
授权集合，后续 action 可在该集合内运行而无需重复询问。路径解析仍在每次调用时执行，防止
相对路径、软链接或模型输出绕过边界。

“能够访问”不等同于“切换工作区”。普通的外部读取、写入或搜索只扩大会话访问范围，保持原有
活动工作区。只有模型明确请求打开一个目录为工作区时，运行时才在必要审批后切换活动根，并重新
加载该目录的项目规则、Git 与 workspace context。
